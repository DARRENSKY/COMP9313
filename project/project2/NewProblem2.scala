...
object Problem2{
  def main(args: Array[String]) = {
    ...
    val graphxPregel = initialGraph.pregel(Set[List[VertexId]](List()), k)(
      // Vertex Program Implementation
      (Indexid, OldAttr, NewAttr) => (for{ linefirst <- OldAttr if linefirst.size <0 } yield linefirst)
        ++
        (for{linesecond <- NewAttr if (!linesecond.contains(Indexid) && linesecond.size < k) || (linesecond.contains(Indexid) && linesecond(0) == Indexid && linesecond.size == k) } yield linesecond),
    ...
    val fourthnew4 = thirdnew3.filter{yline => yline.size==k}.size
